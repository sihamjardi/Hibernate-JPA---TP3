...
public class App {
    ...
    private static void testRelationsEtCascade(EntityManagerFactory emf) {
        ...
        try {
            ...
            // --- 1) Création des entités "indépendantes" ---
            ...
            // Persist des "parents" avant de créer la réservation
            em.persist(utilisateur);
            em.persist(salle);

            // flush() est optionnel ici mais peut aider la visibilité immédiate
            // em.flush();

            // --- 2) Créer la réservation UNE FOIS les parents persistés ---
            ...
            // Établir les relations (utiliser les helpers pour garder la cohérence)
            utilisateur.addReservation(reservation);
            salle.addReservation(reservation);

            // 3 options pour persister la réservation :
            // (a) si Reservation est gérée via cascade depuis Utilisateur => em.persist(utilisateur) aurait suffit
            // (b) ici on persist explicitement la réservation pour être pleinement explicite :
